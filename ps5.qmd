...
6. Start the "for loop" by requesting and extractiong the data from each of the total number of pages, using request and Beautiful Soup. We will extract all the data first, withouth filtering to have filtering to have the most recent one. We will only filter at the end once we have our first preliminary tidy dataframe. We could have filter on each page, but the work would be too tedious and it doesn't reduce computation time that much as we are still going through every page.
  6.1 Inside the 'for loop', Extract the 'titles' for each enforcement and append them to a list.
  6.2 Inside the loop, Extract 'links' associated with each enforcement and append them to a list.
  6.3 Inside the loop, Extract the dates for each enforcement and append them to a list.
  6.4 Inside the loop, Extract the 'categories' for each enforcement and append them to a list.
  ...
  6.5 Inside the loop, Extract the 'agencies' for each enforcement, using the links for the titles, and append them to a list.
7. Append all the generated list to a tidy dataframe
8. Filtering the generated dataframe to have only observation which are as recent as the the month and year provided as arguments.
9. Return the filtered dataframe as the final result of the function


* b. Then we can build the function to create Dynamic Scraper.
